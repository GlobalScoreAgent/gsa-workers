#!/usr/bin/env python3
"""Initial fungible portfolio discovery from wallet_token_contracts + Alchemy + DeFiLlama."""
...
CLAIMED_BY_PREFIX = "wallet_token_portfolio_discovery/gha"
...
async def run_job() -> int:
    ...
    concurrency = env_int("CONCURRENCY", default=5, minimum=1, maximum=15)
    ...
    try:
        async with httpx.AsyncClient(timeout=45.0, limits=http_limits) as http_client:
            while True:
                ...
                if not rows:
                    if processed == 0:
                        logger.info("No pending portfolio discovery rows. Exiting.")
                    else:
                ...
                async def handle_row(row: dict) -> tuple[int, bool, int]:
                    ...
                    async with sem:
                        try:
                            async with db_lock:
                                contracts = db.load_contracts(wallet_id, chain_id)
                            ...
                            return row_id, True, len(positions)
                        except Exception as exc:
