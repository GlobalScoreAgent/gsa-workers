"""Map Goldsky Virtual ACP satellite items → virtual_acp upsert row dicts."""
...
def _job_ref(chain_id: str, job_id: int) -> str:
    return f"{chain_id}-{job_id}"
...
def _common(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "job_ref_id": _job_ref(chain, job_id) if chain else None,
    }
