"""Supabase Postgres access for wallet_lp_positions_discovery job."""
...
WITH candidates AS (
  ...
  WHERE wt.does_need_lp_discovery IS DISTINCT FROM FALSE
  ...
  ORDER BY wt.lp_discovery_claimed_at NULLS FIRST, wt.id
  LIMIT %(limit)s
...
class Database:
    ...
    def upsert_positions_and_mark_done(
        self,
        row_id: int,
        wallet_id: int,
        chain_id: int,
