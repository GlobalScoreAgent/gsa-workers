#!/usr/bin/env python3
"""Initial LP positions discovery (UniV3 NFT + classic pools from wallets.lp_pools)."""
...
CLAIMED_BY_PREFIX = "wallet_lp_positions_discovery/gha"
    ...
    try:
        async with httpx.AsyncClient(timeout=45.0, limits=http_limits) as http_client:
            while True:
                ...
                if not rows:
                    if processed == 0:
                        logger.info("No pending LP discovery rows. Exiting.")
                    else:
                ...
                async def handle_row(row: dict) -> tuple[int, bool, int]:
                    ...
                    async with sem:
                        try:
                            ...
                            async with db_lock:
                                msg = db.upsert_positions_and_mark_done(
                                    row_id=row_id,
                                    wallet_id=wallet_id,
                                    chain_id=chain_id,
                                    positions=positions,
