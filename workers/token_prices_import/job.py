...
def main() -> int:
    ...
    logger.info(
        "Starting token prices import (query_id=%s, page_size=%s)",
        query_id,
        page_size,
    )
    ...
    logger.info("Fetched %s rows from Dune; calling wallets.token_prices_upsert", len(rows))

    db = Database(dsn)
    try:
        db.connect()
        message = db.upsert_token_prices(rows)
    except Exception:
