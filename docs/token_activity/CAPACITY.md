...
Assumptions: base fleet peak ~11 jobs at 0/6/12/18; reserve **2** for enrich → room for **≤8** probe jobs (one per chain).

| Control | Value |
|---------|-------|
| `token_activity_runner_count` | **1 per chain** (matrix ≤8) |
| In-process `CONCURRENCY` | **4** (one claim → N getLogs sub-batches) |
| `strategy.max-parallel` | **8** |
| Cron | **3/9/15/21** UTC |
